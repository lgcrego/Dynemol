 module Hamiltonians
    ...
    use omp_lib
    use constants_m
    use parameters_m          , only : EnvField_ , Induced_ , Environ_type , Environ_step , B_ext , SO_coupl
...
do i = 1 , n
    ...
    do j = i , n
        ...
        tz = D_zero
        if( B_ext( 3 ) /= D_zero ) tz = ( 2.0d0 * dfloat( basis( i ) % m ) + key * g_S * dfloat( basis( i ) % s ) ) * S_matrix( j , i )
