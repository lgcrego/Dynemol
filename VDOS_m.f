...
!
! Alberto Torres
! checked by LGC Rego 01/Sept/2023
!
...
module VDOS_m
    ...
    integer                     :: Nspc,      &    ! Number of species (:= MM % N_of_species)
                                   Natoms,    &
                                   Nsamples,  &
...
!$omp parallel do private(i,nsp) default(shared) reduction(+:v0_norm,v0_norm_mw) if(Natoms > SERIAL_THRESHOLD)
do i = 1, Natoms

    nsp = atom(i) % my_species

    v0(i) % vel = atom(i) % vel + low_prec
    v0_norm(nsp) = v0_norm(nsp) + VEC3_DOT_PROD( v0(i)%vel, v0(i)%vel )
    
    mw_v0(i) % vel = ( v0(i) % vel )*( atom(i) % mass )
    v0_norm_mw(nsp) = v0_norm_mw(nsp) + VEC3_DOT_PROD( mw_v0(i)%vel, v0(i)%vel )
