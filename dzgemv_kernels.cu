...
#define magma_trans_t  cublasOperation_t
#define MagmaTrans     CUBLAS_OP_T
#define MagmaNoTrans   CUBLAS_OP_N
#define MagmaConjTrans CUBLAS_OP_C
...
/// Assumes number of threads <= 2048 (which is max number of threads up to CUDA capability 6.5)
/// Having n as template parameter allows compiler to evaluate some conditions at compile time.
/// Calls __syncthreads before & after reduction.
//  --> Copied from magma_templates.h
template< int n, typename T >
__device__ void
magma_sum_reduce( /*int n,*/ int i, T* x )
{
    __syncthreads();
    if ( n > 2048 ) { if ( i < 2048 && i + 2048 < n ) { x[i] += x[i+2048]; }  __syncthreads(); }
 ...
 */
// __global__ void
// dzgemvn_kernel1_fermi(
//     int m, int n, double alpha,
//     const double * __restrict__ A, int lda,
//     const magmaDoubleComplex * __restrict__ x, int incx, double beta,
...
*/
    ...
    if ( trans == MagmaNoTrans ) {
        ...
        dzgemvn_kernel2_fermi<<< grid, threads, 0, stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == MagmaTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        dzgemvt_kernel_fermi<<< grid, threads, 0, stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == MagmaConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        dzgemvc_kernel_fermi<<< grid, threads, 0, stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
