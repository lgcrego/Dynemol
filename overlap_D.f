...
logical         , optional , intent(in)     :: recycle
 ...
 ! local variables ...
real*8  :: expa, expb, Rab , aux , anor , aux_b, anor_b
real*8  :: sux(0:10)
...
!$omp private(ib,ia,b_coord,a_coord,atom_not_moved,atom_a_not_moved,atom_b_not_moved,Rab,jb,ja,b,a,k,nb,na,lb,la,mb,ma,aux,aux_b,msup,solnorm,j,i,expb,expa,solvec,anor,anor_b,m,sol_partial,sux,rl,rl2)
!$omp do schedule(dynamic,1)
do ib = 1, b_system% atoms

    If( b_system%QMMM(ib) /= "QM") cycle
    ...
    do ia = ib+1 , a_system% atoms  
        ...
        If( motion_detector_ready ) then
            ...
            atom_b_not_moved = all( b_coord == b_xyz(ib,:) )
            atom_not_moved = atom_a_not_moved .and. atom_b_not_moved 
