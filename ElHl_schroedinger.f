...
if( SOC ) then
    ...
    if( not(comb) ) then
        ...
        open( unit = 54 , file = "dyn.trunk/"//"Polarization_mujica.dat" , status = "replace" , action = "write" , position = "append" )
        ...
        do it = 2 , n_t
            ...
            do nf = 1 , nc-1
                p(nf) = QDyn%dyn(it,nf,1,1) + QDyn%dyn(it,nf,1,2) + QDyn%dyn(it,nf,2,1) + QDyn%dyn(it,nf,2,2)
                p(nf) = ( QDyn%dyn(it,nf,1,1) + QDyn%dyn(it,nf,2,1) - QDyn%dyn(it,nf,2,2) - QDyn%dyn(it,nf,1,2) ) / p(nf)
                vec(nf) = vec(nf) + p(nf)
            end do
