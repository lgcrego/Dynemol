    ...
    do i = 1, MM%N_of_molecules
        do j = 1, molecule(i)%Nbonds14
            ...
            if (atom(atj)%flex .OR. atom(ati)%flex) then
                call NON_BONDED_INTRAMOLECULAR_INTERACTIONS(atom(ati), atom(atj))
            end if
...
subroutine NON_BONDED_INTRAMOLECULAR_INTERACTIONS(atom_i, atom_j)
    ...
    atom_i%fnonch14(1:3) = atom_i%fnonch14(1:3) + (freal * rij(:))
    atom_j%fnonch14(1:3) = atom_j%fnonch14(1:3) - (freal * rij(:))
