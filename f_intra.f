...
! local variables ...
 ...
 real*8 :: rijq, rjkq, rklq, rijsq, rjksq, rklsq, fxyz, riju, riku, rijkj, rijkj2, rjkkl, rjkkl2, rijkl2, rjksq2, rijkll,       &
         f1x, f1y, f1z, f2x, f2y, f2z, f3x, f3y, f3z, f4x, f4y, f4z, sr2, sr6, sr12, fs, phi, cosphi, sinphi, rsinphi, coephi,  &
         gamma, KRIJ, expar, eme, dphi, term, chrgi, chrgj, freal, sig, eps, pterm, A0, A1, A2, A3, rtwopi, qterm, rterm, sterm,& 
                ...
                do loop = 1, 3                    !eixos X,Y,Z (n = 1, 2 ou 3)
                    ...
                    fxyz = ( molecule(i) % kang0(j,1) * coephi ) *            &
                        ( (DEL(atl,atj)-DEL(atl,ati))*riku/(rijsq*rjksq) +    &
                         ...
                         - COS(phi)*(                                         &
                          (DEL(atl,atj)-DEL(atl,ati))*riju/(rijsq*rijsq) +    &
...
! selection of potential energy function type
   select case( adjustl(Dihedral_Potential_Type) )
        ...
        case('cos3') ! v = 1/2.A1[1 + cos(phi)] + 1/2.A2[1 - cos(2.phi)] + 1/2.A3[1 + cos(3.phi)]
           ...
           gamma  = 0.5d0 * ( molecule(i) % kdihed0(j,1) * sin(phi)              - &
                             2.d0 * molecule(i) % kdihed0(j,2) * sin(2.d0*phi)   + &
                             3.d0 * molecule(i) % kdihed0(j,3) * sin(3.d0*phi) ) * rsinphi * rijkj * rjkkl 
   ...
   ! Calculate atomic forces ...
       f1x = gamma * ( (-rjkl(2) * rjk(3) + rjkl(3) * rjk(2)) -        &
                       (-rijk(2) * rjk(3) + rijk(3) * rjk(2)) * coephi * rijkj2)
       f1y = gamma * ( ( rjkl(1) * rjk(3) - rjkl(3) * rjk(1)) -        &
                       ( rijk(1) * rjk(3) - rijk(3) * rjk(1)) * coephi * rijkj2)
       f1z = gamma * ( (-rjkl(1) * rjk(2) + rjkl(2) * rjk(1)) -        &
                       (-rijk(1) * rjk(2) + rijk(2) * rjk(1)) * coephi * rijkj2)
       f3x = gamma * ( (-rjkl(2) * rij(3) + rjkl(3) * rij(2)) -        &
                       (-rijk(2) * rij(3) + rijk(3) * rij(2)) * coephi * rijkj2)
       f3y = gamma * ( ( rjkl(1) * rij(3) - rjkl(3) * rij(1)) -        &
                       ( rijk(1) * rij(3) - rijk(3) * rij(1)) * coephi * rijkj2)
       f3z = gamma * ( (-rjkl(1) * rij(2) + rjkl(2) * rij(1)) -        &
                       (-rijk(1) * rij(2) + rijk(2) * rij(1)) * coephi * rijkj2)
       f2x = gamma * ( (-rijk(2) * rkl(3) + rijk(3) * rkl(2)) -        &
                       (-rjkl(2) * rkl(3) + rjkl(3) * rkl(2)) * coephi * rjkkl2)
       f2y = gamma * ( ( rijk(1) * rkl(3) - rijk(3) * rkl(1)) -        &
                       ( rjkl(1) * rkl(3) - rjkl(3) * rkl(1)) * coephi * rjkkl2)
       f2z = gamma * ( (-rijk(1) * rkl(2) + rijk(2) * rkl(1)) -        &
                       (-rjkl(1) * rkl(2) + rjkl(2) * rkl(1)) * coephi * rjkkl2)
       f4x = gamma * ( (-rijk(2) * rjk(3) + rijk(3) * rjk(2)) -        &
                       (-rjkl(2) * rjk(3) + rjkl(3) * rjk(2)) * coephi * rjkkl2)
       f4y = gamma * ( ( rijk(1) * rjk(3) - rijk(3) * rjk(1)) -        &
                       ( rjkl(1) * rjk(3) - rjkl(3) * rjk(1)) * coephi * rjkkl2)
       f4z = gamma * ( (-rijk(1) * rjk(2) + rijk(2) * rjk(1)) -        &
                       (-rjkl(1) * rjk(2) + rjkl(2) * rjk(1)) * coephi * rjkkl2)
