...
IF( .NOT. done ) then 
    ...
    write(10,'(''ERFC damping (Wolf)       :'',F10.2,'' Angs^-1'')'   ) KAPPA
...
open (10, file='log.trunk/MM_log.out', status='unknown', access='append')
    ...
    write(10,'(''DWFF Potential              :'',F14.4)') DWFF_intra*mol*1.d-6    
...
!
!=======================================
subroutine GenerateConfigs( frame , dt )
!=======================================
implicit none
integer , intent(in) :: frame
real*8  , intent(in) :: dt
...
OPEN(unit=8 , file=f_name , action='write', status='unknown')
     ! saving atomic velocities for configurations generation ... 
     do i = 1 , MM % N_of_atoms
         write(8,*) atom(i) % vel(1),  atom(i) % vel(2), atom(i) % vel(3) 
     end do
close(8)
 
CALL ReGroupMolecule

f_name = 'ancillary.trunk/configs/input-'//string//'.pdb'

open (14 , file=f_name , action='write', status='unknown' )
     write(14,995) 'TITLE'  , ' generated by Dynemol spawn ', thermostat
     write(14,991) 'CRYST1' , MM % box(1) , MM % box(2) , MM % box(3) , 90.0 , 90.0 , 90.0 , 'P 1' , '1'
...
write( * , 240 )  counter, frame , frame*dt*1.d12

counter = counter + 1

991 FORMAT(a6,3F9.3,3F7.2,a11,a4)
992 FORMAT('ATOM  ',i5,a5,a1,a3,a2,i4,a4,3F8.3,2F6.2,a4,a6,a2,F8.4,t90,a3)
995 FORMAT(a5,t15,a29,t50,a15)

include 'formats.h' 
       
end subroutine GenerateConfigs
!
!
!
